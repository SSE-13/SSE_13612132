module astar {
    ...
    export class Grid {
        ...
        private _nodes: Array<Array<Node>>; //方块数组
        private _numCols: number;
        private _numRows: number;

        constructor(numCols, numRows) {
            this._numCols = numCols; //x
            this._numRows = numRows; //y
            this._nodes = [];
        ...
        public getNode(x: number, y: number): Node {
            return this._nodes[x][y]; //得到坐标
        }

        public setStartNode(x: number, y: number): void {
            this._startNode = this._nodes[x][y]; //起始位置
        }

        public setEndNode(x: number, y: number): void {
            this._endNode = this._nodes[x][y]; //终点
        }

        public setWalkable(x, y, value): void {
            this._nodes[x][y].walkable = value; //设置是否可以通过
        }
        public getWalkable(x, y, value): Boolean {
            return this._nodes[x][y].walkable;
        } 

        public get startNode(): Node {
            return this._startNode; //得到起始
        }

        public get endNode(): Node {
            return this._endNode; //得到终点
        }

        public get numCols(): number {
            return this._numCols; //x
        }

        public get numRows(): number {
            return this._numRows; //y
        }
